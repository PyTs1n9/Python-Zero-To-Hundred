...
data_path = base_dir / "data" / "student_health_dirty.csv"
...
clean.to_csv(output_dir / "student_health_clean.csv", index=False, encoding="utf-8-sig")
scaled.to_csv(output_dir / "student_health_scaled.csv", index=False, encoding="utf-8-sig")
model_data.to_csv(output_dir / "student_health_model_data.csv", index=False, encoding="utf-8-sig")
standard_check.to_csv(output_dir / "student_health_standardization_check.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "student_health_cleaning_report.csv", index=False, encoding="utf-8-sig")

# 按比赛提交风格说明处理顺序、规则和产物。
notes = f"""# 数据清洗说明

1. 原始数据：student_health_dirty.csv，共 {original_rows} 行。
...
"""
(output_dir / "student_health_cleaning_notes.md").write_text(notes, encoding="utf-8")
