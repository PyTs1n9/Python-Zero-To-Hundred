...
data_path = base_dir / "data" / "book_sales_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['price', 'sales_count']
categorical_fields = ['category', 'qualified']
df = pd.read_csv(data_path, dtype={"sale_id": str})
...
clean.to_csv(output_dir / "book_sales_clean.csv", index=False, encoding="utf-8-sig")
pd.DataFrame(bounds_rows).to_csv(output_dir / "book_sales_iqr_bounds.csv", index=False, encoding="utf-8-sig")
before_summary.to_csv(output_dir / "book_sales_summary_before.csv", index=False, encoding="utf-8-sig")
after_summary.to_csv(output_dir / "book_sales_summary_after.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "book_sales_cleaning_report.csv", index=False, encoding="utf-8-sig")
