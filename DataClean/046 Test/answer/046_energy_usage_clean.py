...
data_path = base_dir / "data" / "energy_usage_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['electricity', 'gas', 'peak_load']
categorical_fields = ['region', 'qualified']
df = pd.read_csv(data_path, dtype={"meter_id": str})
...
clean.to_csv(output_dir / "energy_usage_clean.csv", index=False, encoding="utf-8-sig")
pd.DataFrame(bounds_rows).to_csv(output_dir / "energy_usage_iqr_bounds.csv", index=False, encoding="utf-8-sig")
before_summary.to_csv(output_dir / "energy_usage_summary_before.csv", index=False, encoding="utf-8-sig")
after_summary.to_csv(output_dir / "energy_usage_summary_after.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "energy_usage_cleaning_report.csv", index=False, encoding="utf-8-sig")
