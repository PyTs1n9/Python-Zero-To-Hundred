...
data_path = base_dir / "data" / "bank_clients_private.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

sensitive_fields = ["name", "phone", "id_card"]
numeric_fields = ['age', 'income', 'loan_amount', 'credit_score', 'default_risk']
...
partial_df.to_csv(output_dir / "bank_clients_masked_partial.csv", index=False, encoding="utf-8-sig")
full_df.to_csv(output_dir / "bank_clients_masked_full.csv", index=False, encoding="utf-8-sig")
sha_df.to_csv(output_dir / "bank_clients_hash_sha256.csv", index=False, encoding="utf-8-sig")
md5_df.to_csv(output_dir / "bank_clients_hash_md5.csv", index=False, encoding="utf-8-sig")
...
hash_report.to_csv(output_dir / "bank_hash_consistency_report.csv", index=False, encoding="utf-8-sig")
...
stats.to_csv(output_dir / "bank_stats_comparison.csv", index=False, encoding="utf-8-sig")
...
pearson_before.to_csv(output_dir / "bank_pearson_before.csv", encoding="utf-8-sig")
pearson_after.to_csv(output_dir / "bank_pearson_after.csv", encoding="utf-8-sig")
...
heatmap(pearson_before, "Pearson Correlation Before Desensitization", "bank_pearson_before_heatmap.png")
heatmap(pearson_after, "Pearson Correlation After Desensitization", "bank_pearson_after_heatmap.png")
...
validation.to_csv(output_dir / "bank_privacy_validation_report.csv", index=False, encoding="utf-8-sig")

analysis = f"""# 隐私脱敏分析报告

## 数据与敏感字段

数据文件为 bank_clients_private.csv，共 {len(df)} 行；name、phone、id_card 为模拟敏感字段。
...
"""
(output_dir / "bank_privacy_analysis_report.md").write_text(analysis, encoding="utf-8")
