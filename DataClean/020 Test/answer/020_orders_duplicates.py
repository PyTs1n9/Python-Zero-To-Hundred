...
data_path = base_dir / "data" / "orders_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

df = pd.read_csv(data_path, dtype={"order_id": str, "phone": str})
...
key_duplicate_records = int(no_full.duplicated(subset=["order_id"], keep=False).sum())
clean_key = no_full.drop_duplicates(subset=["order_id"], keep="first").copy()

no_full.to_csv(output_dir / "orders_no_full_duplicates.csv", index=False, encoding="utf-8-sig")
clean_key.to_csv(output_dir / "orders_clean_keep_first.csv", index=False, encoding="utf-8-sig")
...
report = pd.DataFrame([
    ...
    {"metric": "order_id重复组涉及记录数量", "value": key_duplicate_records},
    {"metric": "按order_id保留第一条后的行数", "value": len(clean_key)},
...
report.to_csv(output_dir / "orders_duplicate_report.csv", index=False, encoding="utf-8-sig")
