...
data_path = base_dir / "data" / "ecommerce_sales_missing.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

# 明确目标字段，避免误修改编号等无需填充的字段。
numeric_fields = ['price', 'sales_count', 'rating']
categorical_fields = ['category']
fixed_values = {'price': 0, 'sales_count': 0, 'rating': 3.0}
...
mean_df.to_csv(output_dir / "ecommerce_sales_filled_mean.csv", index=False, encoding="utf-8-sig")
median_df.to_csv(output_dir / "ecommerce_sales_filled_median.csv", index=False, encoding="utf-8-sig")
mixed_df.to_csv(output_dir / "ecommerce_sales_filled_mode_fixed.csv", index=False, encoding="utf-8-sig")
comparison.to_csv(output_dir / "ecommerce_sales_fill_mean_comparison.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "ecommerce_sales_missing_report.csv", index=False, encoding="utf-8-sig")
