...
data_path = base_dir / "data" / "lab_samples_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['ph', 'concentration', 'temperature', 'purity']
categorical_fields = ['sample_type', 'qualified']
...
clean.to_csv(output_dir / "lab_samples_clean.csv", index=False, encoding="utf-8-sig")
pd.DataFrame(bounds_rows).to_csv(output_dir / "lab_samples_iqr_bounds.csv", index=False, encoding="utf-8-sig")
before_summary.to_csv(output_dir / "lab_samples_summary_before.csv", index=False, encoding="utf-8-sig")
after_summary.to_csv(output_dir / "lab_samples_summary_after.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "lab_samples_cleaning_report.csv", index=False, encoding="utf-8-sig")
