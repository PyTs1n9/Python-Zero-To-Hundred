...
data_path = base_dir / "data" / "vehicle_inspection_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['vehicle_age', 'mileage_km', 'emission']
...
clean.to_csv(output_dir / "vehicle_inspection_clean.csv", index=False, encoding="utf-8-sig")
scaled.to_csv(output_dir / "vehicle_inspection_scaled.csv", index=False, encoding="utf-8-sig")
model_data.to_csv(output_dir / "vehicle_inspection_model_data.csv", index=False, encoding="utf-8-sig")
standard_check.to_csv(output_dir / "vehicle_inspection_standardization_check.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "vehicle_inspection_cleaning_report.csv", index=False, encoding="utf-8-sig")

# 按比赛提交风格说明处理顺序、规则和产物。
notes = f"""# 数据清洗说明

1. 原始数据：vehicle_inspection_dirty.csv，共 {original_rows} 行。
...
"""
(output_dir / "vehicle_inspection_cleaning_notes.md").write_text(notes, encoding="utf-8")
