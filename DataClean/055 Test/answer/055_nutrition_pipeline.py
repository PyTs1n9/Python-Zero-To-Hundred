...
data_path = base_dir / "data" / "nutrition_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['calories', 'protein', 'fat', 'carbohydrate']
...
clean.to_csv(output_dir / "nutrition_clean.csv", index=False, encoding="utf-8-sig")
scaled.to_csv(output_dir / "nutrition_scaled.csv", index=False, encoding="utf-8-sig")
model_data.to_csv(output_dir / "nutrition_model_data.csv", index=False, encoding="utf-8-sig")
standard_check.to_csv(output_dir / "nutrition_standardization_check.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "nutrition_cleaning_report.csv", index=False, encoding="utf-8-sig")

# 按比赛提交风格说明处理顺序、规则和产物。
notes = f"""# 数据清洗说明

1. 原始数据：nutrition_dirty.csv，共 {original_rows} 行。
...
6. 标准化：对 calories, protein, fat, carbohydrate 使用 StandardScaler。
7. 结果：清洗后 {len(clean)} 行，缺失值 {int(clean.isna().sum().sum())} 个。
"""
(output_dir / "nutrition_cleaning_notes.md").write_text(notes, encoding="utf-8")
