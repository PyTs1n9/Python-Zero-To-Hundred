...
data_path = base_dir / "data" / "inventory_missing.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

# 明确目标字段，避免误修改编号等无需填充的字段。
numeric_fields = ['stock_count', 'turnover_days']
categorical_fields = ['warehouse']
fixed_values = {'stock_count': 0, 'turnover_days': 0}
...
mean_df.to_csv(output_dir / "inventory_filled_mean.csv", index=False, encoding="utf-8-sig")
median_df.to_csv(output_dir / "inventory_filled_median.csv", index=False, encoding="utf-8-sig")
mixed_df.to_csv(output_dir / "inventory_filled_mode_fixed.csv", index=False, encoding="utf-8-sig")
comparison.to_csv(output_dir / "inventory_fill_mean_comparison.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "inventory_missing_report.csv", index=False, encoding="utf-8-sig")
