...
data_path = base_dir / "data" / "factory_quality.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

fields = ['length', 'width', 'weight', 'hardness', 'defect_rate']
...
pd.DataFrame(comparison_rows).to_csv(output_dir / "factory_quality_outlier_detection_comparison.csv", index=False, encoding="utf-8-sig")
df.loc[z_any].to_csv(output_dir / "factory_quality_zscore_outliers.csv", index=False, encoding="utf-8-sig")
clean_delete.to_csv(output_dir / "factory_quality_clean_delete.csv", index=False, encoding="utf-8-sig")
clean_clip.to_csv(output_dir / "factory_quality_clean_clip.csv", index=False, encoding="utf-8-sig")
clean_replace.to_csv(output_dir / "factory_quality_clean_replace.csv", index=False, encoding="utf-8-sig")
treatment.to_csv(output_dir / "factory_quality_treatment_comparison.csv", index=False, encoding="utf-8-sig")
