...
data_path = base_dir / "data" / "hotel_rooms_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['price', 'occupancy_rate', 'rating']
categorical_fields = ['room_type', 'qualified']
df = pd.read_csv(data_path, dtype={"room_record_id": str})
...
clean.to_csv(output_dir / "hotel_rooms_clean.csv", index=False, encoding="utf-8-sig")
pd.DataFrame(bounds_rows).to_csv(output_dir / "hotel_rooms_iqr_bounds.csv", index=False, encoding="utf-8-sig")
before_summary.to_csv(output_dir / "hotel_rooms_summary_before.csv", index=False, encoding="utf-8-sig")
after_summary.to_csv(output_dir / "hotel_rooms_summary_after.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "hotel_rooms_cleaning_report.csv", index=False, encoding="utf-8-sig")
