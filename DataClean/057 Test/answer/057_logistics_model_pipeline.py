...
data_path = base_dir / "data" / "logistics_model_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['distance', 'weight', 'volume', 'delivery_hours']
...
clean.to_csv(output_dir / "logistics_model_clean.csv", index=False, encoding="utf-8-sig")
scaled.to_csv(output_dir / "logistics_model_scaled.csv", index=False, encoding="utf-8-sig")
model_data.to_csv(output_dir / "logistics_model_model_data.csv", index=False, encoding="utf-8-sig")
standard_check.to_csv(output_dir / "logistics_model_standardization_check.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "logistics_model_cleaning_report.csv", index=False, encoding="utf-8-sig")

# 按比赛提交风格说明处理顺序、规则和产物。
notes = f"""# 数据清洗说明

1. 原始数据：logistics_model_dirty.csv，共 {original_rows} 行。
...
"""
(output_dir / "logistics_model_cleaning_notes.md").write_text(notes, encoding="utf-8")
