...
data_path = base_dir / "data" / "logistics_missing.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

# 明确目标字段，避免误修改编号等无需填充的字段。
numeric_fields = ['distance_km', 'delivery_hours', 'freight']
categorical_fields = ['route_type']
fixed_values = {'distance_km': 0, 'delivery_hours': 0, 'freight': 0}
...
mean_df.to_csv(output_dir / "logistics_filled_mean.csv", index=False, encoding="utf-8-sig")
median_df.to_csv(output_dir / "logistics_filled_median.csv", index=False, encoding="utf-8-sig")
mixed_df.to_csv(output_dir / "logistics_filled_mode_fixed.csv", index=False, encoding="utf-8-sig")
comparison.to_csv(output_dir / "logistics_fill_mean_comparison.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "logistics_missing_report.csv", index=False, encoding="utf-8-sig")
