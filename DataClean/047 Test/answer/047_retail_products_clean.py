...
data_path = base_dir / "data" / "retail_products_dirty.csv"
output_dir = base_dir / "output"
output_dir.mkdir(parents=True, exist_ok=True)

numeric_fields = ['price', 'stock', 'monthly_sales']
categorical_fields = ['category', 'qualified']
df = pd.read_csv(data_path, dtype={"product_id": str})
...
clean.to_csv(output_dir / "retail_products_clean.csv", index=False, encoding="utf-8-sig")
pd.DataFrame(bounds_rows).to_csv(output_dir / "retail_products_iqr_bounds.csv", index=False, encoding="utf-8-sig")
before_summary.to_csv(output_dir / "retail_products_summary_before.csv", index=False, encoding="utf-8-sig")
after_summary.to_csv(output_dir / "retail_products_summary_after.csv", index=False, encoding="utf-8-sig")
report.to_csv(output_dir / "retail_products_cleaning_report.csv", index=False, encoding="utf-8-sig")
